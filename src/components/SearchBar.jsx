    ...
    useEffect(() => {
        const handler = setTimeout(async () => {
            if (value.trim().length >= 2) {
                try {
                    const res = await stationApi.search(value)
                    // Get unique cities from suggestions
                    const cities = [...new Set(res.data.map(s => s.city))]
        ...
        <div className="relative flex-1" ref={ref}>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="input-field pl-9 text-white"
                    onFocus={() => suggestions.length > 0 && setShow(true)}
                    autoComplete="off"
                />
            </div>
            {show && suggestions.length > 0 && (
                <ul className="absolute top-full mt-2 left-0 right-0 bg-dark-700 border border-white/10 rounded-xl overflow-hidden z-50 shadow-2xl animate-fade-in">
    ...
    return (
        <form onSubmit={handleSubmit} className="glass-card p-6">
            ...
            <div className="flex flex-col lg:flex-row gap-3 items-end">
                {/* Origin */}
                <CityInput label="From" value={origin} onChange={setOrigin} placeholder="Origin city" />
                ...
                {/* Destination */}
                <CityInput label="To" value={destination} onChange={setDestination} placeholder="Destination city" />
